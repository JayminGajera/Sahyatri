  ...
  return (
    <div className="absolute top-12 left-0 bg-gray-800 h-[88vh] md:h-fit w-full md:w-full rounded-md">
            <div className="flex flex-col justify-center items-start p-5 gap-y-5">
              ...
              <p >{user.accountType === "Driver" ? <span >vehicle No : <span className="opacity-70">{user?.vehicleNumber}</span></span>   : " "} </p>
              <p >{user.accountType === "Driver" ? <span >Driving Licence No : <span className="opacity-70">{user?.drivingLicence}</span></span>   : " "} </p>
              <p >{user.accountType === "Driver" ? <span >Aadhar Card No : <span className="opacity-70">{user?.photoId}</span></span>   : " "} </p>
